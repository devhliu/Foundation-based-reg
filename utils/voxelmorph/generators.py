...
def vol_seg_gen(
    ...
    if isinstance(seg_names, str):
        if os.path.isdir(seg_names):
            seg_names = os.path.join(seg_names, '*')
        seg_names = glob.glob(seg_names)
    ...
    while True:
        ...
        seg = [py.utils.load_volfile(seg_names[i], **load_params) for i in indices]

        segVols = [np.concatenate(seg, axis=0)]
        ...
        # print('pre',type(vols))
        # yield vols, segVols
        yield tuple(vols), tuple(segVols) #original voxelmorph wraps tuple for unknown reasons

def vol_seg_multiModalGen(
    vol_names,
    seg_names,
    batch_size=1,
    segs=None,
    ...
    Parameters:
        ...
        add_feat_axis: Load volume arrays with added feature axis. Default is True.
    """
    # 'C:\Users\Bwhite\Desktop\voxelmorph - dev\voxelmorph - dev\task1_data\Train'

    # convert glob path to filenames
    # currently tuned for task1 dataset
    if isinstance(vol_names, str):
        if os.path.isdir(vol_names):
            vol_names = os.path.join(vol_names, 'img*')
        vol_names = glob.glob(vol_names)
    if isinstance(seg_names, str):
        if os.path.isdir(seg_names):
            seg_names = os.path.join(seg_names, 'seg*')
        seg_names = glob.glob(seg_names)

    volMR_names = sorted([s for s in vol_names if "MR" in s])
    volCT_names = sorted([s for s in vol_names if "CT" in s])
    segMR_names = sorted([s for s in seg_names if "MR" in s])
    segCT_names = sorted([s for s in seg_names if "CT" in s])
    # print(len(volMR_names))
    # print(len(volCT_names))
    # print(len(segMR_names))
    # print(len(segCT_names))

    if isinstance(segs, list) and len(segs) != len(vol_names):
        raise ValueError('Number of image files must match number of seg files.')

    print('CT dataset length = ', len(volCT_names))
    print('MR dataset length = ', len(volMR_names))
    index = 0
    while True:
        # generate [batchsize] random image indices
        indicesMR = np.random.randint(len(volMR_names), size=batch_size)
        if unpaired:
            indicesCT = np.random.randint(len(volCT_names), size=batch_size)
        else:
            #for validation, only 3 samples
            indicesMR = [index%3]
            index +=1
            indicesCT = indicesMR
        ...
        imgMR = [py.utils.load_volfile(volMR_names[i], **load_params) for i in indicesMR]
        volsMR = [np.concatenate(imgMR, axis=0)]
        # if not unpaired:
        #     print(volCT_names[indicesCT[0]])
        imgCT = [py.utils.load_volfile(volCT_names[i], **load_params) for i in indicesCT]
        volsCT = [np.concatenate(imgCT, axis=0)]

        segMR = [py.utils.load_volfile(segMR_names[i], **load_params) for i in indicesMR]
        volsSegMR = [np.concatenate(segMR, axis=0)]
        segCT = [py.utils.load_volfile(segCT_names[i], **load_params) for i in indicesCT]
        volsSegCT = [np.concatenate(segCT, axis=0)]
        ...
        # print('pre',type(vols))
        # yield vols, segVols
        yield tuple(volsMR), tuple(volsCT), tuple(volsSegMR), tuple(volsSegCT) #original voxelmorph wraps tuple for unknown reasons
...
def scan_to_scan_Seg(vol_names, seg_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    Generator for scan-to-scan registration.
    ...
    # sys.exit()
    # gen = volgen(vol_names, batch_size=batch_size, **kwargs)
    gen = vol_seg_gen(vol_names=vol_names,seg_names=seg_names,np_var='seg', batch_size=batch_size, **kwargs)
    while True:
        ...
        scan1,seg1 = next(gen)
        scan1,seg1 = scan1[0], seg1[0]
        # print('post',type(scan1))

        scan2,seg2 = next(gen)
        scan2,seg2 = scan2[0], seg2[0]
        ...
        yield (invols, outvols, inseg)


def scan_to_scan_task1(vol_names, seg_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    Generator for scan-to-scan registration.
    ...
    # sys.exit()
    # gen = volgen(vol_names, batch_size=batch_size, **kwargs)
    gen = vol_seg_multiModalGen(vol_names=vol_names, seg_names=seg_names, np_var='seg', batch_size=batch_size, **kwargs)
    while True:

        # scan1,seg1 = next(gen)
        # scan2,seg2 = next(gen)

        scanMR, scanCT, segMR, segCT = next(gen)
        scanMR, scanCT, segMR, segCT = scanMR[0], scanCT[0], segMR[0], segCT[0]

        # cache zeros
        if not no_warp and zeros is None:
            shape = scanMR.shape[1:-1]
            zeros = np.zeros((batch_size, *shape, len(shape)))

        invols = [scanCT, scanMR] # moving image is CT
        # outvols = [scanMR, scanCT] if bidir else [scanMR]
        inseg = [segCT, segMR]
